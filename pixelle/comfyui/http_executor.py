    ...
    async def _queue_prompt(self, workflow: Dict[str, Any], client_id: str, prompt_ext_params: Optional[Dict[str, Any]] = None) -> str:
        ...
        for attempt in range(MAX_RETRY_ATTEMPTS):
            try:
                ...
                logger.warning(f"Attempt {attempt + 1}/{MAX_RETRY_ATTEMPTS} to queue prompt failed: {e}")
                if attempt < MAX_RETRY_ATTEMPTS - 1:
                    await asyncio.sleep(RETRY_INTERVAL_SECONDS)
                else:
                    logger.error(f"Failed to queue prompt after {MAX_RETRY_ATTEMPTS} attempts.")
                ...
                try:
                    async with self.get_comfyui_session() as session:
                        async with session.get(history_url) as response:
                            if response.status != 200:
                                raise Exception(f"Failed to get history: [{response.status}] {await response.text()}")
                            
                            history_data = await response.json()
                            if prompt_id not in history_data:
                                raise Exception(f"Prompt ID {prompt_id} not in history data.")
                            
                            prompt_history = history_data[prompt_id]
                            status = prompt_history.get("status")
                            if status and status.get("status_str") == "error":
                                break # Exit retry loop if error status
                            
                            if "outputs" in prompt_history:
                                # If outputs are present, we have a result, break retry loop
                                break
                except Exception as e:
                    logger.warning(f"Attempt {attempt + 1}/{MAX_RETRY_ATTEMPTS} to get history for prompt {prompt_id} failed: {e}")
                    if attempt < MAX_RETRY_ATTEMPTS - 1:
                        await asyncio.sleep(RETRY_INTERVAL_SECONDS)
                    else:
                        logger.error(f"Failed to get history for prompt {prompt_id} after {MAX_RETRY_ATTEMPTS} attempts.")
                        raise # Re-raise the last exception if all attempts fail
            
            # After retry loop, check if we have history data
            if prompt_id not in history_data:
                await asyncio.sleep(1.0) # Original sleep, keep it for polling
                continue
